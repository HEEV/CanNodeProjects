...
void tempRTR(CanMessage *data); 
int16_t tempCalc(uint16_t voltage);
/// Struct for initilizing ADC
ADC_HandleTypeDef hadc;
/// Struct for transmitting the thermocouple temp
CanNode *thermistorNode;

//variable used to hold the thermocouple volage if any
int16_t thermocoupleVoltage;
...
int main(void) {
  ...
  thermocoupleVoltage = 0;
  //USBConnected = false;
  //this is going to hold the value of the temperature when it is fully calculated
  ...
  CanNode temp_couple(TEMPURATURE, tempRTR); 
  thermistorNode = &temp_couple;
  //set up object that is going to be used too pass the temperature
  /*
...
*/
  while (1) {
    ...
    if (time % 250 == 0) {
	  ...
	  voltage=adcVal;
	  finalTemp=tempCalc(voltage);
	  thermistorNode->sendData_int16(finalTemp);
	}
		/*
      // do some heavy math
...
`	// used for debugging with usb	 


	 strcat(buff, "temp data:");
      CDC_Transmit_FS((uint8_t *)buff, strlen(buff));
	  
	  
	  itoa(finalTemp, buff, 10);
	  strcat(buff, "\n");
...
*/
	...
	// begin temparature reading FIXME
	if(time % 300 == 0){
...
int16_t tempCalc(uint16_t adcVal){
	//Define the voltages for the thermocouple datasheet given
	const float A = 0.0033540164;
	const float B = 0.000300131;
	const float C = 5.08516E-06;
	const float parrRes = 8.33;
	
	float voltage=(adcVal/4096.0)*3.3; //get the volatge off of the adc
	
	float r = parrRes * ((3.3 / voltage) - 1.0);
	float R = log(r/10);
	float invTemp = A + B * R + C *(R*R);
	float temperature = ((1.0 / invTemp)-273.15)*100;
	//voltage=(uint16_t)voltage; If voltage is needed
	finalTemp=(int16_t)temperature;//change the global variable
	return (int16_t) temperature; 
}


//rtr handeler for the temperature
void tempRTR(CanMessage *data){
  thermistorNode->sendData_int16(finalTemp);
}
